...
async function provideHover(model, position, sclHelper) {
  ...
  try {
    ...
    if (!response || !response.markdownStrings) {
      return undefined;
    }

    var contents = [];

    for (let markdownString of response.markdownStrings) {
      contents.push({ value: markdownString });
    }
      ...
      for (let signature of res.signatures) {
        ...
        for (let parameter of signature.parameters) {
          let parameterInfo = {
            ...
            documentation: {kind: 'Markdown', value:parameter.documentation }
          }
